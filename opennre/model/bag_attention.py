...
class BagAttention(BagRE):
    ...
    def forward(self, label, scope, token, pos1, pos2,pics, mask=None, train=True, bag_size=0):
        """
        Args:
        ...
        if mask is not None:
            rep = self.sentence_encoder(token, pos1, pos2, mask,pics) # (nsum, H)
        else:
            rep = self.sentence_encoder(token, pos1, pos2,pics) # (nsum, H)
